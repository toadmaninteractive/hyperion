...
squery(PoolName, Statement) ->
    Result = poolboy:transaction(PoolName, fun(Worker) ->
        gen_server:call(Worker, {squery, Statement}, db_config:timeout())
    end, db_config:timeout()),
    log_error(Result),
    Result.
...
equery(PoolName, Statement, Params) ->
    Result = poolboy:transaction(PoolName, fun(Worker) ->
        gen_server:call(Worker, {equery, Statement, Params}, db_config:timeout())
    end, db_config:timeout()),
    log_error(Result),
    Result.
...
transaction(Arg) ->
    Reply = poolboy:transaction(?db_pool, fun(Worker) ->
        gen_server:call(Worker, {transaction, Arg}, db_config:timeout())
    end, db_config:timeout()),
    case Reply of
