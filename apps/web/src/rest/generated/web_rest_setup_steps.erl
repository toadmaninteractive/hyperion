...
-module(web_rest_setup_steps).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    create_setup_step_400/1,
    create_setup_step_403/1,
    create_setup_step_500/1
]).
...
create_setup_step_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

create_setup_step_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

create_setup_step_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_post(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_setup:create_setup_step_request_from_json(jsx:decode(RequestBody, [return_maps])),
        ProjectId = igor_http:parse_value(cowboy_req:binding(project_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_setup:create_setup_step(RequestContent, ProjectId, Req1),
