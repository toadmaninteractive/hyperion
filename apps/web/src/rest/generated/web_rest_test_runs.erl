...
-module(web_rest_test_runs).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    create_test_run_400/1,
    create_test_run_403/1,
    create_test_run_500/1
]).
...
create_test_run_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

create_test_run_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

create_test_run_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_post(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_test_run:create_test_run_request_from_json(jsx:decode(RequestBody, [return_maps])),
        ProjectId = igor_http:parse_value(cowboy_req:binding(project_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_test_run:create_test_run(RequestContent, ProjectId, Req1),
        ...
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_test_run:create_test_run_error_to_json/1})),
