...
-module(web_rest_attachment_links).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    get_attachment_links_403/1,
    get_attachment_links_500/1
]).
...
get_attachment_links_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

get_attachment_links_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_get(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        ResponseContent = web_rest_callback_attachment:get_attachment_links(Id),
        Body = jsx:encode(protocol_data:collection_to_json(ResponseContent, {custom, fun protocol_db:file_attachment_link_to_json/1})),
