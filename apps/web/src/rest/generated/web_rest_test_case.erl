...
-export([
    ...
    get_test_case_404/1,
    get_test_case_500/1,
    update_test_case_400/1,
    update_test_case_403/1,
    update_test_case_404/1,
    update_test_case_500/1,
    delete_test_case_400/1,
    delete_test_case_403/1,
    delete_test_case_404/1,
    delete_test_case_500/1
]).
...
update_test_case_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

update_test_case_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

update_test_case_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

update_test_case_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_test_case:update_test_case_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_test_case:update_test_case(RequestContent, Id, Req1),
        Body = jsx:encode(protocol_db:test_case_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
...
delete_test_case_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

delete_test_case_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

delete_test_case_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

delete_test_case_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_delete(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_test_case:delete_test_case(Id, Req),
