...
get_setup_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

get_setup_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_get(Req) ->
    try
        ProjectId = igor_http:parse_value(cowboy_req:binding(project_id, Req), long),
        ResponseContent = web_rest_callback_setup:get_setup(ProjectId),
        Body = jsx:encode(protocol_data:collection_to_json(ResponseContent, {custom, fun(V) -> protocol_data:tree_node_to_json(V, {custom, fun protocol_db:setup_step_to_json/1}) end})),
