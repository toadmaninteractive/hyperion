...
-module(web_rest_parameter_value_rename).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    rename_parameter_value_400/1,
    rename_parameter_value_403/1,
    rename_parameter_value_404/1,
    rename_parameter_value_500/1
]).
...
rename_parameter_value_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

rename_parameter_value_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

rename_parameter_value_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

rename_parameter_value_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_parameter:rename_parameter_value_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_parameter:rename_parameter_value(RequestContent, Id, Req1),
        ...
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_parameter:parameter_value_rename_error_to_json/1})),
