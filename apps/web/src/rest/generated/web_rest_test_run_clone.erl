...
-module(web_rest_test_run_clone).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    clone_test_run_400/1,
    clone_test_run_403/1,
    clone_test_run_404/1,
    clone_test_run_500/1
]).
...
clone_test_run_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

clone_test_run_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

clone_test_run_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

clone_test_run_500(ResponseContent500) ->
...
handle_put(Req) ->
    try
        ...
        {ResponseContent, Req2} = web_rest_callback_test_run:clone_test_run(RequestContent, Id, Req1),
        ...
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_test_run:clone_test_run_error_to_json/1})),
