...
-module(web_rest_jira_auth).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    get_authentication_status_403/1,
    get_authentication_status_404/1,
    get_authentication_status_500/1,
    authenticate_400/1,
    authenticate_403/1,
    authenticate_404/1,
    authenticate_500/1,
    revoke_authentication_403/1,
    revoke_authentication_404/1,
    revoke_authentication_500/1
]).
...
handle_method(<<"DELETE">>, Req) ->
    handle_delete(Req);
handle_method(_, Req) ->
    ResponseHeaders = #{<<"Allow">> => <<"GET, POST, DELETE">>},
    cowboy_req:reply(405, ResponseHeaders, Req).

get_authentication_status_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

get_authentication_status_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

get_authentication_status_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_get(Req) ->
    try
        JiraId = igor_http:parse_value(cowboy_req:binding(jira_id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_jira:get_authentication_status(JiraId, Req),
...
authenticate_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

authenticate_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

authenticate_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

authenticate_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_post(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_jira:authenticate_jira_request_from_json(jsx:decode(RequestBody, [return_maps])),
        JiraId = igor_http:parse_value(cowboy_req:binding(jira_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_jira:authenticate(RequestContent, JiraId, Req1),
        Body = jsx:encode(protocol_data:generic_response_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
        cowboy_req:reply(200, ResponseHeaders, Body, Req2)
    catch
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_jira:jira_authentication_error_to_json/1})),
...
revoke_authentication_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

revoke_authentication_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

revoke_authentication_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_delete(Req) ->
    try
        JiraId = igor_http:parse_value(cowboy_req:binding(jira_id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_jira:revoke_authentication(JiraId, Req),
        Body = jsx:encode(protocol_data:generic_response_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
        cowboy_req:reply(200, ResponseHeaders, Body, Req1)
    catch
