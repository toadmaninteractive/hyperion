...
-export([
    ...
    get_project_404/1,
    get_project_500/1,
    update_project_400/1,
    update_project_403/1,
    update_project_404/1,
    update_project_500/1,
    delete_project_403/1,
    delete_project_404/1,
    delete_project_500/1
]).
...
update_project_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

update_project_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

update_project_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

update_project_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_project:update_project_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_project:update_project(RequestContent, Id, Req1),
        Body = jsx:encode(protocol_db:project_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
...
delete_project_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

delete_project_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

delete_project_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_delete(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_project:delete_project(Id, Req),
