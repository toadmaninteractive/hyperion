...
-module(web_rest_test_case_specialize).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    specialize_test_case_400/1,
    specialize_test_case_403/1,
    specialize_test_case_404/1,
    specialize_test_case_500/1
]).
...
specialize_test_case_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

specialize_test_case_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

specialize_test_case_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

specialize_test_case_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_test_case:specialize_test_case_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        SetupId = igor_http:parse_value(cowboy_req:binding(setup_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_test_case:specialize_test_case(RequestContent, Id, SetupId, Req1),
        ...
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_test_case:test_case_specialize_error_to_json/1})),
