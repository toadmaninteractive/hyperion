...
-module(web_rest_test_run_item).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    get_test_run_item_403/1,
    get_test_run_item_404/1,
    get_test_run_item_500/1,
    update_test_run_item_400/1,
    update_test_run_item_403/1,
    update_test_run_item_404/1,
    update_test_run_item_500/1,
    delete_test_run_item_403/1,
    delete_test_run_item_404/1,
    delete_test_run_item_500/1
]).
...
update_test_run_item_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

update_test_run_item_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

update_test_run_item_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

update_test_run_item_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_test_run:update_test_run_item_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_test_run_item:update_test_run_item(RequestContent, Id, Req1),
        Body = jsx:encode(protocol_db:test_run_item_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
        cowboy_req:reply(200, ResponseHeaders, Body, Req2)
    catch
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_test_run:update_test_run_item_error_to_json/1})),
...
delete_test_run_item_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

delete_test_run_item_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

delete_test_run_item_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_delete(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_test_run_item:delete_test_run_item(Id, Req),
