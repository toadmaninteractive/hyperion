...
-module(web_rest_setup_step_param_link).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    link_setup_step_to_parameter_400/1,
    link_setup_step_to_parameter_403/1,
    link_setup_step_to_parameter_404/1,
    link_setup_step_to_parameter_500/1
]).
...
link_setup_step_to_parameter_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

link_setup_step_to_parameter_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

link_setup_step_to_parameter_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

link_setup_step_to_parameter_500(ResponseContent500) ->
...
handle_put(Req) ->
    try
        ...
        ParamId = igor_http:parse_value(cowboy_req:binding(param_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_setup:link_setup_step_to_parameter(RequestContent, Id, ParamId, Req1),
        ...
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_setup:setup_link_error_to_json/1})),
