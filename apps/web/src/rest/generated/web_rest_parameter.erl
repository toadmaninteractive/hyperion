...
-export([
    ...
    get_parameter_404/1,
    get_parameter_500/1,
    rename_parameter_400/1,
    rename_parameter_403/1,
    rename_parameter_404/1,
    rename_parameter_500/1,
    delete_parameter_400/1,
    delete_parameter_403/1,
    delete_parameter_404/1,
    delete_parameter_500/1
]).
...
rename_parameter_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

rename_parameter_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

rename_parameter_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

rename_parameter_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_parameter:rename_parameter_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_parameter:rename_parameter(RequestContent, Id, Req1),
        Body = jsx:encode(protocol_db:parameter_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
        cowboy_req:reply(200, ResponseHeaders, Body, Req2)
    catch
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_parameter:parameter_rename_error_to_json/1})),
...
delete_parameter_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

delete_parameter_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

delete_parameter_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

delete_parameter_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_delete(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_parameter:delete_parameter(Id, Req),
        Body = jsx:encode(protocol_data:generic_response_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
        cowboy_req:reply(200, ResponseHeaders, Body, Req1)
    catch
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_parameter:parameter_delete_error_to_json/1})),
            ResponseHeaders400 = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
            cowboy_req:reply(400, ResponseHeaders400, ResponseContent400Body, Req);
        #{status_code := 403, response := ResponseContent403} ->
            ResponseContent403Body = jsx:encode(protocol_data:forbidden_error_to_json(ResponseContent403)),
