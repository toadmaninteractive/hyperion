...
-module(web_rest_test_run_item_start).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    start_test_run_item_400/1,
    start_test_run_item_403/1,
    start_test_run_item_404/1,
    start_test_run_item_500/1
]).
...
start_test_run_item_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

start_test_run_item_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

start_test_run_item_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

start_test_run_item_500(ResponseContent500) ->
...
handle_put(Req) ->
    try
        ...
        {ResponseContent, Req2} = web_rest_callback_test_run_item:start_test_run_item(RequestContent, Id, Req1),
        ...
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_test_run:start_test_run_item_error_to_json/1})),
