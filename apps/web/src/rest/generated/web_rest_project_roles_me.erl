...
-export([
    init/2,
    get_my_roles_for_project_403/1,
    get_my_roles_for_project_404/1,
    get_my_roles_for_project_500/1
]).
...
get_my_roles_for_project_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

get_my_roles_for_project_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

get_my_roles_for_project_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_get(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {Response, Req1} = web_rest_callback_admin_personnel_roles:get_my_roles_for_project(Id, Req),
        Body = jsx:encode(protocol_db:personnel_account_role_to_json(Response)),
