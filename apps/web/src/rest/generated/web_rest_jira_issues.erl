...
-module(web_rest_jira_issues).

-include_lib("igor/include/igor_http.hrl").

-export([
    init/2,
    create_jira_issue_400/1,
    create_jira_issue_403/1,
    create_jira_issue_404/1,
    create_jira_issue_500/1
]).
...
create_jira_issue_400(ResponseContent400) ->
    throw(#{status_code => 400, response => ResponseContent400}).

create_jira_issue_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

create_jira_issue_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

create_jira_issue_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_post(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = protocol_jira:create_jira_issue_request_from_json(jsx:decode(RequestBody, [return_maps])),
        TestRunItemId = igor_http:parse_value(cowboy_req:binding(test_run_item_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_jira:create_jira_issue(RequestContent, TestRunItemId, Req1),
        Body = jsx:encode(protocol_jira:create_jira_issue_response_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
        cowboy_req:reply(200, ResponseHeaders, Body, Req2)
    catch
        #{status_code := 400, response := ResponseContent400} ->
            ResponseContent400Body = jsx:encode(protocol_data:bad_request_error_to_json(ResponseContent400, {custom, fun protocol_jira:jira_issue_error_to_json/1})),
