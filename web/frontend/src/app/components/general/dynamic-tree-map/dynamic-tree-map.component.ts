import { AfterContentInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ContentChild, ElementRef, EventEmitter, Input, OnChanges, OnInit, Output, TemplateRef } from '@angular/core';
...
@Component({
    selector: 'm-dynamic-tree-map',
    templateUrl: 'dynamic-tree-map.component.html',
    styleUrls: ['dynamic-tree-map.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class DynamicTreeMapComponent implements OnInit, AfterContentInit {
    ...
    @Input() inputNodes$ = new BehaviorSubject<Map<number, TreeNode<SetupStep | TestCase | Parameter>>>(null);
    @Input() flatNodes = new Map<number, SetupStep | TestCase | Parameter>(null);
    flatDynamicNodes = new Map<number, DynamicNode<SetupStep | TestCase | Parameter>>(null);
    ...
    @Output() nodeAdded = new EventEmitter<number | boolean>();
    ...
    @Output() treeRendered = new EventEmitter<boolean>();

    checkedNodeSet = new Set<number>();
    indeterminateNodeSet  = new Set<any>();

    nodes: Map<number, DynamicNode<TestCase | SetupStep | Parameter>> = new Map();
    ...
    ngAfterContentInit() {
        ...
        this.ready$.pipe(filter(r => !!r)).subscribe(r => this.treeRendered.emit(true));
    ...
    ngOnInit() {
        this.inputNodes$
            ...
            .subscribe(value => {
                ...
                this.nodes.clear();
                value.forEach(node => this.nodes.set(node.item.id, new DynamicNode(node, this.expandedNodeSet)));
                this.nodes.forEach(node => this.walkDynamicTreeNodes(node));
    ...
    public expandTreeNodes(nodes: Map<number, DynamicNode<TestCase | SetupStep | Parameter>>
                               | Array<DynamicNode<TestCase | SetupStep | Parameter>> = this.nodes): void {
        nodes.forEach(node => {
            node.expanded = true;
    ...
    public expandTreeNodesById(nodesId: Array<number>): void {
        this.flatDynamicNodes.forEach(node => {
            if (nodesId.includes(node.item.id)) {
                node.expanded = true;
            }
        });
        this.cdr.detectChanges();
    ...
    public collapseTreeNodes(nodes: Map<number, DynamicNode<TestCase | SetupStep | Parameter>>
        | Array<DynamicNode<TestCase | SetupStep | Parameter>> = this.nodes): void {
        nodes.forEach(node => {
            node.expanded = false;
    ...
    private walkDynamicTreeNodes(nodes: Array<DynamicNode<TestCase | SetupStep | Parameter>>
                                    | DynamicNode<TestCase | SetupStep | Parameter>): void {
        if (nodes instanceof DynamicNode) {
            this.flatDynamicNodes.set(nodes.item.id, nodes);
            this.walkDynamicTreeNodes(nodes.children);
        } else {
            nodes.forEach(node => {
                this.flatDynamicNodes.set(node.item.id, node);
                this.walkDynamicTreeNodes(node.children);
            });
        }
    ...
    private saveExpandedNode(nodes: Map<number, DynamicNode<TestCase | SetupStep | Parameter>>
                                    | Array<DynamicNode<TestCase | SetupStep | Parameter>>): void {
        if (nodes instanceof Map) {
            nodes.forEach(node => {
                if (node.expanded) {
                    this.expandedNodeSet.add(node.item.id);
                }
            });
    ...
    private checkDraftChildren(nodes: Map<number, DynamicNode<TestCase | SetupStep | Parameter>>
                                    | Array<DynamicNode<TestCase | SetupStep | Parameter>>): void {

        if (nodes instanceof Map) {
            nodes.forEach(node => {
                if (node.children.find(child => child.item.id === 0)) {
                    this.hasDraftChildrenSet.add(node.item.id);
                }
            });
        } else if (nodes instanceof Array) {
    ...
    //         const parentNode = this.flatNodes.get(node.item.parentId);
    ...
    verifyChildrenNode(dragged: DynamicNode<TestCase| SetupStep | Parameter>, target: DynamicNode<TestCase| SetupStep | Parameter>): boolean {
        return target.item.parentId === dragged.item.id
            ...
            : target.item.parentId
                ? this.verifyChildrenNode(dragged, this.flatDynamicNodes.get(target.item.parentId))
                : true;
    }
